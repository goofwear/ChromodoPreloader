...
/////////////////////////////////////////////////////////////////////////////
// Finds the path to Chromodo in the registry using the key for the latest version
/////////////////////////////////////////////////////////////////////////////
// pszChromodoPathBuffer can be NULL to simply test if we can even find
// a valid path to Chromodo.
...
BOOL GetCurrentChromodoProfile(CString *pszProfilePathBuffer) {
	...
	// Trim the \ off if it is there and add the Mozilla\Chromodo
	if (szADataPath[szADataPath.GetLength() - 1] == _T('\\')) szADataPath.TrimRight(_T('\\'));
	szADataPath += "\\Mozilla\\Chromodo";
	...
	::OutputDebugString(_T("Found the Mozilla Chromodo profiles.ini\r\n"));
	CloseHandle(hUseful);
...
BOOL GetLatestChromodoInstallDir(CString *pszChromodoPathBuffer) {
	...
	CRegistryKey *moziKey = cReg.KeyLocalMachine()->OpenKey(_T("SOFTWARE\\Mozilla\\Mozilla Chromodo"), KEY_READ);
	...
	::OutputDebugString(_T("Found Mozilla Chromodo's root key\r\n"));
		...
		if (mozvKey != NULL) {
			...
			if (fCurrentVer > fLastVer) {
				...
				if (mozvKey->GetStringValue(_T("Install Directory"), &szCurrentPath) == ERROR_SUCCESS) {
					...
					// Make the chromodo.exe path
					szFFExe = szCurrentPath;
					szFFExe += _T("\\chromodo.exe");

					// See if chromodo.exe exists in the path by opening it, only then do we have the right key
					...
					if (hFFExe != INVALID_HANDLE_VALUE) {
						::OutputDebugString(_T(": chromodo.exe Found!"));

						// Update latest version, and set that path
						fLastVer = fCurrentVer;
						szPath = szCurrentPath;
						CloseHandle(hFFExe);
					} else ::OutputDebugString(_T(": chromodo.exe not found or invalid"));				
				} else ::OutputDebugString(_T(": No \"Install Directory\""));
			} // fCurrentVer > fLastVeChromodo
	...
	if (pszChromodoPathBuffer != NULL) *pszChromodoPathBuffer = szPath;
...
// Load and Wait for Chromodo working thread function
UINT LoadAndWaitForChromodo(LPVOID pParam) {
	CString										// String Variables
		szFF,									// chromodo.exe path and command-line
	...
	if (!mutex.Lock(1000)) {
		::OutputDebugString(_T("Unable to lock loading Mutex, Chromodo may still be loaded\r\n"));
		return FALSE;
	}

	// Create the command-line to start Chromodo
	szFF = pDlg->p_szChromodoPath;
	szFF += _T("\\chromodo.exe -chrome \"chrome://browser/content/web-panels.xul\"");
	...
	// Load Chromodo, hidden
	if (!::CreateProcess(NULL, szFF.GetBuffer(szFF.GetLength()), NULL, NULL, TRUE, NORMAL_PRIORITY_CLASS, NULL, pDlg->p_szChromodoPath, &si, &pi)) {
		::OutputDebugString(_T("Chromodo failed to run...\r\n"));
