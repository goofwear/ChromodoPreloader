...
BEGIN_MESSAGE_MAP(CDefaultDlg, CDialog)
	...
	ON_COMMAND(ID_CONTEXTMENU_UNLOADChromodoPRELOADER, OnContextmenuUnloadChromodopreloader)
	ON_COMMAND(ID_CONTEXTMENU_INTERNETOPTIONS, OnContextmenuInternetoptions)
	ON_COMMAND(ID_CONTEXTMENU_ABOUTChromodoPRELOADER, OnContextmenuAboutChromodopreloader)
	ON_COMMAND(ID_CONTEXTMENU_UNLOADChromodo, OnContextmenuUnloadChromodo)
	ON_COMMAND(ID_CONTEXTMENU_RELOADChromodo, OnContextmenuReloadChromodo)
...
// Load Chromodo if it is not loaded
...
BOOL CDefaultDlg::OnInitDialog() {
	...
	CRegistryKey *softKey = this->p_cReg.KeyCurrentUser()->CreateKey(_T("SOFTWARE\\6XGate Incorporated\\ChromodoPreload"));
	...
	// Load the AutoFind configuration, then get Chromodo's path if AutoFind is enabled
	softKey->GetIntegerValue(_T("AutoFind"), (ULONG*)&(this->p_bAutoFindFF));
	if (this->p_bAutoFindFF && ::GetLatestChromodoInstallDir(&szLPath)) {
		::OutputDebugString(_T("Using AutoFind\r\n"));

		// Remove the saved path since AutoFind found Chromodo's path
		softKey->DeleteValue(_T("Path"));
		this->p_szChromodoPath = szLPath;		
	...
	// If AutoFind is not enabled, or the path couldn't be found, use the manually entered path
		...
		softKey->GetStringValue(_T("Path"), &(this->p_szChromodoPath));
	}

	// If the path was not found by the manual setting or automatically, ask the user for it
	if (this->p_szChromodoPath.IsEmpty()) {
	...
	this->p_bUserLock = ::GetCurrentChromodoProfile(&(this->p_szProfilePath));

	// Show the tray icon, free the key, and load Chromodo
	this->ShowTrayIcon();
	delete softKey;
	this->PreloadFF();
	...
	if(message == AM_TRAYNOTIFY) {
		if (wParam == 5000) {
			...
			case WM_RBUTTONUP:
				...
				break;
			// Open a new Chromodo window with the users homepage when double-clicked
...
void CDefaultDlg::OnContextmenuAboutChromodopreloader() { CAboutDlg dlgAbout; dlgAbout.DoModal(); }	// About...

void CDefaultDlg::OnContextmenuOpenmyhomepage() {
	CString szFF;								// chromodo.exe path and command-line
	STARTUPINFO si; PROCESS_INFORMATION pi;		// Create process structures

	// Set the path to chromodo.exe so that it will open a new window with the user's homepage
	szFF = this->p_szChromodoPath;
	szFF += _T("\\chromodo.exe");
	...
	// Load a new Chromodo with the user's homepage
	::CreateProcess(NULL, szFF.GetBuffer(szFF.GetLength()), NULL, NULL, FALSE, 0, NULL, this->p_szChromodoPath, &si, &pi);
}
...
void CDefaultDlg::OnContextmenuInternetoptions() {
	...
	// Set the path to chromodo.exe so that it will open the Chromodo Options dialog
	szFF = this->p_szChromodoPath;
	szFF += _T("\\chromodo.exe -chrome \"chrome://browser/content/pref/pref.xul\"");
	...
	// Load the Chromodo Options dialog
	::CreateProcess(NULL, szFF.GetBuffer(szFF.GetLength()), NULL, NULL, FALSE, 0, NULL, this->p_szChromodoPath, &si, &pi);
}

void CDefaultDlg::OnContextmenuReloadChromodo() {
	CString			// String Variables
		szMessage,	// MessageBox message resources
		szTitle;	// MessageBox title resources

	// Only unload Chromodo if the Preloader is active
	if (this->p_bTimer) {
		if (this->p_bWarnOnUnloads) {
			// Warn the user
			::OutputDebugString(_T("Warning the user that we will unload Chromodo\r\n"));
		...
		// Kill the timer so it doesn't keep trying to start Chromodo, restart it when Chromodo is loaded
	...
	this->SetTimer(1, this->p_nTimerInt, NULL);
	this->p_bTimer = TRUE;
	this->GetMenu()->GetSubMenu(0)->EnableMenuItem(ID_CONTEXTMENU_UNLOADChromodo, MF_BYCOMMAND | MF_ENABLED);
	this->ChangeTrayIconState(TRUE);
}

void CDefaultDlg::OnContextmenuUnloadChromodo() {
	CString			// String Variables
		szMessage,	// MessageBox message resources
		szTitle;	// MessageBox title resources
	...
	// Kill the time and unload Chromodo
	if (this->p_bTimer) {
		this->KillTimer(1);
		this->p_bTimer = FALSE;
	}
	this->UnloadFF();
	this->GetMenu()->GetSubMenu(0)->EnableMenuItem(ID_CONTEXTMENU_UNLOADChromodo, MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
	this->ChangeTrayIconState(FALSE);
}

void CDefaultDlg::OnContextmenuUnloadChromodopreloader() {
	CString			// String Variables
		szMessage,	// MessageBox message resources
		szTitle;	// MessageBox title resources

	// Only unload Chromodo if the Preloader was active
	if (this->p_bTimer) {
		if (this->p_bWarnOnUnloads) {
			// Warn the user
			::OutputDebugString(_T("Warning the user that we will unload Chromodo\r\n"));
		...
		// Unload the hidden Chromodo and end the program
		this->UnloadFF();
	}
...
int CDefaultDlg::ShowPreloaderOptionsDlg() {
	...
	dlg.m_szPath = this->p_szChromodoPath;			// Current Chromodo Path
	dlg.m_bAutoFindFF = this->p_bAutoFindFF;		// Do we AutoFind Chromodo?
	dlg.m_bWarnOnUnload = this->p_bWarnOnUnloads;	// Do we warn the user about unloading Chromodo?
	
	// Show the dialog
	int nResponse = dlg.DoModal();
	if (nResponse == IDOK) {
		// If the user enables AutoFind, try to find Chromodo automaticly
		if (dlg.m_bAutoFindFF && ::GetLatestChromodoInstallDir(&szLPath)) {
		...
		// If the user manually entered the path that was validated, use it
			...
			this->p_bAutoFindFF = FALSE;
			this->p_szChromodoPath = dlg.m_szPath;

			// Trim the \ off the path if it is present save the settings
			if (this->p_szChromodoPath[this->p_szChromodoPath.GetLength() - 1] == _T('\\')) this->p_szChromodoPath.TrimRight(_T('\\'));
			softKey->SetStringValue(_T("Path"), this->p_szChromodoPath);
			softKey->SetIntegerValue(_T("AutoFind"), FALSE);
...
// Triple-redundant method to see if Chromodo is running
BOOL CDefaultDlg::IsChromodoRunning() {
	CString szLockFile;		// The path to the lock file
	
	// First, check for the ChromodoMessageWindow
	if (::FindWindow(_T("ChromodoMessageWindow"), NULL) != NULL) return TRUE;
	::OutputDebugString(_T("Didn't find ChromodoMessageWindow\r\n"));	
	...
	// Finally, check to see if chromodo.exe is present in the currently running processes
	if (::IsProcessActive(_T("chromodo.exe"))) return TRUE;
	::OutputDebugString(_T("Didn't chromodo.exe process\r\n"));
	return FALSE;

	// If all three atemps to find Chromodo failled, it must not be loaded
	return FALSE;
}

BOOL CDefaultDlg::PreloadFF() {

	// Make sure that Chromodo is not running
	if (this->IsChromodoRunning()) return TRUE;

	// Executing pre-loading thread
	AfxBeginThread(LoadAndWaitForChromodo, (LPVOID)this, 0, 0, 0, NULL);
	return FALSE;
}
...
BOOL CDefaultDlg::UnloadFF() {
	...
	// Find and close Windows with the ChromodoMessageWindow class, this ensures proper termination of Chromodo with all of its
	// settings saved
	hFWnd = ::FindWindow(_T("ChromodoMessageWindow"), NULL);
	while (hFWnd != NULL) {
